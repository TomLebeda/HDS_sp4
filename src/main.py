...
fakes = [
    ...
    "kolik je obsazených míst v novém divadle",
    "kolik bylo před dvěma hodinama volných míst v rychtářce",
    "jaká je kapacita",
    "nového divadla",
    ...
    async def loop(self):
        ...
        while True:
            if self.state is State.INIT:
                ...
                print(f'------------SLU RESULT: {slu.entities}')
                self.temp += 1
                ...
                elif self.question is Question.ARE_AT_LEAST:
                    logging.debug(f'processing question: ARE_AT_LEAST')
                    self.state = State.IDLE

                elif self.question is Question.WHERE_IS_MORE:
                    logging.debug(f'processing question: WHERE_IS_MORE')
                    self.state = State.IDLE
    ...
    def extract_time(self, slu: EntityMap) -> bool:
        ...
        try:
            ...
            if chunks[0] == 'add':
                ...
                self.time.replace(hour=int(chunks[0]), minute=int(chunks[2]))
            return True
        except:
            return False
