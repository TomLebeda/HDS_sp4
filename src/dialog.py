...
class SpeechCloudWS(websocket.WebSocketHandler, EventEmitter):
    ...
    def open(self):
        gen_log.info(
            'Creating dialog manager as instance of %s', self.dialog_class
        )
        self._task = None
    ...
    async def _init_API_schema(self):
        ...
        await self.write_message(
            {'type': 'sc_activated', 'schema': self.dm_schema}
        )
    ...
    def run(
        ...
        if static_path is not None:
            app_config.append(
                (static_route, MyStaticFileHandler, {'path': static_path})
            )
...
class Dialog(object):
            ...
            while True:
                ...
                if speech_task in done:
                    # wait for asr_result with max_speech_timeout
                        ...
                        {result_task}, timeout=max_speech_timeout
                    )
    ...
    async def wait_for_slu_result(self, timeout=None):
        ...
        slu_result = await self._wait_for_result(
            self._wait_for_slu_entities, timeout
        )
    ...
    async def _synthesize_and_wait_for_result(
        self, awaitable_method, text, timeout=None, **kwargs
    ):
        await self.synthesize_and_wait(text, **kwargs)

        self.logger.debug(
            'Sleeping for RTT delay: %.3f [s]', self.sc.rtt_delay
        )
    ...
    async def synthesize_and_wait_for_asr_result(
        self, text, timeout=None, **kwargs
    ):
        return await self._synthesize_and_wait_for_result(
            self.wait_for_asr_result, text, timeout, **kwargs
        )

    async def synthesize_and_wait_for_slu_result(
        self, text, timeout=None, **kwargs
    ):
        return await self._synthesize_and_wait_for_result(
            self.wait_for_slu_result, text, timeout, **kwargs
        )
    ...
    async def define_slu_grammars(self, grammars):
        ...
        slu_entities = {
            grm['entity']: grm.pop('mapping', None) for grm in grammars
        }

        await self.sc.slu_set_grammars(grammars=grammars)

        done, pending = await asyncio.wait(
            {task_done, task_error}, return_when=asyncio.FIRST_COMPLETED
        )
    ...
    async def use_slu_grammars(self, entity_dict_or_list):
        ...
        slu_entities = {
            entity: mapping
            for (entity, mapping) in entity_dict_or_list.items()
        }
        self._slu_entities = slu_entities
    ...
    async def _main(self, start_session_message):
        ...
        self.logger.debug(
            'Started DM session %s, session log: %s?format=yaml.html',
            self.session_id,
            self.session_uri,
        )
        self.logger.debug(
            'SpeechCloud application schema: %s?format=docson', self.schema_uri
        )
